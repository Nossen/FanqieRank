...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="fanqierank", description="Generate Fanqie male new-book rank reports.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    collect_parser = subparsers.add_parser("collect", help="Collect raw Fanqie male new-book rank data.")
    ...
    if args.command == "collect":
        ...
        print(f"Collected {len(snapshot.categories)} categories for {snapshot.date}")
        return 0

    if args.command == "fallback-finalize":
        result = finalize_report_with_fallback_analysis(args.date, args.output)
        print(f"Fallback-finalized {result['category_count']} categories for {result['date']}")
        return 0

    if args.command == "finalize":
        result = finalize_report_from_analysis(args.date, args.output, args.analysis_file)
        print(f"Finalized {result['category_count']} Codex-analyzed categories for {result['date']}")
        return 0
    ...
    if args.command == "run":
        ...
        print(f"Generated {result['category_count']} categories for {result['date']}")
        return 0
